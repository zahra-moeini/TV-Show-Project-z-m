...
function Card(url, title, summary) {
  // container div
  ...
  h4.classList.add("h4");
  h4.innerHTML = `<span>name</span> : ${title}`; //
  cbody.appendChild(h4);
  // paragraph
  const p = document.createElement("p");
  p.classList.add("card-text");
  p.innerHTML = `<b>summary :</b> ${summary}`;
  cbody.appendChild(p);
  p.classList.add('p')
  return container;
}
// Fetches the pi address and repeats it through the api to be created for each movie and card
...
search.addEventListener("keyup", () => {
  ...
  const Cards = document.getElementById("content").childNodes.length;
  searchMatches.innerHTML = `displaying ${Cards}/73 episodes`;
});
